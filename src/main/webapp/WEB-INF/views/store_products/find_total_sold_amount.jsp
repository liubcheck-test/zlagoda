...
<html>
<head>
    <title>Find Sales by Product and Period</title>
</head>
<body>
<h1 class="table_dark">Find Sales by Product and Period</h1>
<form method="post" action="${pageContext.request.contextPath}/store_products/find-total-sold-amount">
    <label for="upc">Store Product:</label>
    <select name="upc" id="upc" required>
        <c:forEach var="product" items="${products}">
            <option value="${product.upc}" ${product.upc eq upc ? 'selected' : ''}>${product.product.name}</option>
        </c:forEach>
    </select>
    <br><br>
...
<c:if test="${totalAmount > 0}">
    <h3 class="table_dark">Total Sum: ${totalAmount}</h3>
</c:if>
